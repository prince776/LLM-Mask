...
interface AbuseTokenSetupModalProps {
  ...
  onRestoreFromFile: (password: string, file: File) => Promise<void>
...
export const AbuseTokenSetupModal: React.FC<AbuseTokenSetupModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'restore-file' && !selectedFile) { setError('Please select a backup file.'); return }
    ...
    try {
      if (mode === 'setup') {
        ...
        await onRestoreFromFile(password, selectedFile!)
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-[#1c2333] rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-900 dark:text-white">Anonymous Access Setup</h2>
        ...
        {alreadyIssued && (
          <div className="flex gap-2.5 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/40 rounded-xl p-3.5 mb-5">
            ...
            <p className="text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
              Your anonymous access tokens were already issued. Restore them from your backup file or from the server if you enabled server sync.
            </p>
          </div>
        )}
        ...
        {mode === 'setup' && (
          <div className="flex gap-2.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/40 rounded-xl p-3.5 mb-5">
            ...
            <p className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
              Your backup file will be downloaded to your device. Store it safely — you'll need it to restore access on a new device or after clearing browser data.
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {/* File picker (restore-file mode) */}
          {mode === 'restore-file' && (
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                <FolderOpen size={11} className="inline mr-1" />
                Backup file (.llmmaskbak)
              </label>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  className="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/10 transition-colors"
                >
                  Choose File
                </button>
          ...
          <div>
            ...
            <input
              ...
              placeholder="••••••••"
              className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
              required
              autoFocus={mode !== 'restore-file'}
            />
          </div>
          ...
          {mode === 'setup' && (
            <label className="flex items-start gap-2.5 cursor-pointer select-none">
              <div className="relative flex-shrink-0 mt-0.5">
                <input type="checkbox" checked={uploadToServer} onChange={(e) => setUploadToServer(e.target.checked)} className="sr-only" />
                <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${uploadToServer ? 'bg-blue-600 border-blue-600' : 'border-gray-300 dark:border-white/20'}`}>
                  {uploadToServer && (
              ...
              <div>
                <p className="text-xs font-medium text-gray-700 dark:text-gray-300">Also sync encrypted backup to server</p>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">Encrypted with your password before upload. Enables "Restore from Server" on new devices.</p>
              </div>
            </label>
          )}
          ...
          >
            {loading
              ...
              : (mode === 'setup' ? 'Generate & Save Backup' : mode === 'restore-file' ? 'Restore from File' : 'Restore from Server')
            }
          </button>
        </form>
        ...
        {mode === 'setup' && (
          <p className="text-[11px] text-gray-400 dark:text-gray-500 text-center mt-4 leading-relaxed">
            Your backup file will be downloaded locally. The server never sees your unblinded tokens.
          </p>
        )}
        {mode === 'restore-server' && (
          <p className="text-[11px] text-gray-400 dark:text-gray-500 text-center mt-4 leading-relaxed">
