      ...
      >
        <div className="flex flex-col items-start min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${providerColor(selectedModelData?.provider)}`}>
              ...
              return (
                <div key={provider}>
                  <div className="px-3 pt-3 pb-1">
                    <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md ${providerColor(provider)}`}>
                      {provider}
                    </span>
                  </div>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-3 py-2.5 text-left flex items-center justify-between gap-3 transition-colors ${
                          isSelected ? 'bg-blue-50 dark:bg-blue-500/10' : 'hover:bg-gray-50 dark:hover:bg-white/[0.04]'
                        }`}
                      >
                        <div className="flex flex-col min-w-0">
                          <span className={`text-[13px] font-semibold leading-snug ${isSelected ? 'text-blue-700 dark:text-blue-300' : 'text-gray-800 dark:text-gray-100'}`}>
                            {model.name}
                          </span>
