...
interface ChatInputProps {
  onSendMessage: (payload: { text: string; images?: ImageAttachment[]; displayText?: string }) => void
  disabled?: boolean
  isLoading?: boolean
  disabledText?: string
}
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent): void => {
    ...
    const displayText = [text, imageMarkdown].filter(Boolean).join(text && imageMarkdown ? '\n\n' : '')
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div
          className={`relative flex items-end gap-2 bg-gray-50 dark:bg-white/[0.04] rounded-2xl border transition-all duration-150 ${
            disabled || isLoading
              ...
              : 'border-gray-200 dark:border-white/[0.08] focus-within:border-blue-500/50 dark:focus-within:border-blue-500/30'
          }`}
        ...
        >
          ...
          <textarea
            ...
            disabled={disabled || isLoading}
            rows={1}
            className="flex-1 py-3 pr-1 bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none resize-none text-sm leading-relaxed disabled:cursor-not-allowed"
