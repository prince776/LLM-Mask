...
import { getFeedback, sendFeedback } from '../api/feedback'
...
export function FeedbackWidget() {
  ...
  const fetchEntries = async (): Promise<ThreadEntry[] | undefined> => {
    ...
    if (resp.error) { setError(resp.error); return undefined }
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (open) return
      const resp = await getFeedback().catch(() => null)
  ...
  const handleSend = async () => {
    ...
    const optimistic: ThreadEntry = { Role: 'user', Content: text, CreatedAt: new Date().toISOString() }
    setEntries((prev) => [...prev, optimistic])
    setInputText('')

    try {
      const resp = await sendFeedback(text)
      if (resp.error) {
        setError(resp.error)
      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-white/[0.06] flex-shrink-0">
            <span className="text-sm font-semibold text-gray-900 dark:text-white">Feedback &amp; Support</span>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 min-h-0">
            {!user ? (
              <p className="text-sm text-gray-400 dark:text-gray-500 text-center mt-10">Sign in to send feedback</p>
            ) : loading ? (
              <p className="text-sm text-gray-400 dark:text-gray-500 text-center mt-10">Loading...</p>
            ) : entries.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-gray-500 text-center mt-10">Send us a message</p>
            ) : (
              entries.map((entry, i) =>
                entry.Role === 'user' ? (
                  ...
                  <div key={i} className="flex justify-start">
                    <div className="max-w-[85%]">
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 ml-1 mb-0.5">Support</p>
