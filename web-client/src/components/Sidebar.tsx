      ...
      >
        ...
        <div className="flex-shrink-0 px-3 pt-4 pb-3 border-b border-white/[0.06]">
          <div className="flex items-center gap-2 px-1 mb-3">
            <div className="w-6 h-6 rounded-md overflow-hidden flex-shrink-0 bg-blue-600 flex items-center justify-center">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-white">
                <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
            <span className="text-sm font-semibold text-white tracking-tight">LLMTor</span>
            {/* Web indicator instead of Tor indicator */}
            <div className="ml-auto flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-amber-400" />
              <span className="text-[10px] font-semibold text-amber-400 uppercase tracking-widest">WEB</span>
            </div>
            <button
              onClick={onToggle}
              ...
              {chats.map((chat) => (
                <div
                  ...
                  className={`
                    ...
                    ${activeChat === chat.id ? 'bg-white/10 text-white' : 'text-gray-500 hover:bg-white/[0.05] hover:text-gray-300'}
                  `}
                  onClick={() => onChatSelect(chat.id)}
                >
                  <div className="flex-1 min-w-0 pr-1">
                    <p className={`text-[13px] font-medium truncate leading-snug ${activeChat === chat.id ? 'text-gray-100' : ''}`}>
                      {chat.title}
                    </p>
